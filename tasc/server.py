    ...
    def compute_power_accel(self, lever_notch: int, v: float) -> float:
        ...
        if self.veh.T_max_kN > 0.0 and self.veh.P_max_kW > 0.0:
            ...
            if self.veh.maxSpeed_kmh >= 200: 
                ...
                # P1이 30% 힘으로 밀어주므로 출발이 시원시원함
                # torque_table = [
                #     0.30, 0.38, 0.45,  # P1~P3: 초반 힘 강력
                #     0.51, 0.57, 0.63,  # P4~P6
                ...
                # power_table = [
                # # [저속/입환 영역] - 아주 미세함
                # 0.03, 0.07, 0.12,  # P1 ~ P3 : 고속에서 P1~3은 사실상 '타력 주행'에 가까운 느낌
                
                # # [중속/크루징 영역] - 허리를 살짝 보강하되 P7을 0.55로 억제
                # 0.20, 0.32, 0.45,  # P4 ~ P6 : 시내 주행이나 서행 구간에서 적절
                # 0.55,              # P7 : 여기가 핵심! (기존 0.70은 너무 셈 -> 0.55로 하향)
                
                # # [고속 가속 영역] - 고속에서는 출력이 급격히 필요함
                # 0.68, 0.80,        # P8 ~ P9 : 200km/h 대에서 주력으로 사용
                
                # # [초고속 피크] - 300km/h 도달을 위해 마지막에 몰아줌
                # 0.88, 0.94, 0.98, 1.00 # P10 ~ P13
                # ]
